-- ==========================================
-- COMPLETE USER SYSTEM REBUILD
-- ==========================================

-- Step 1: Drop all existing RLS policies for user-related tables
...
-- Step 2: Drop existing functions that reference user roles
DROP FUNCTION IF EXISTS is_super_admin(uuid);
DROP FUNCTION IF EXISTS can_create_clinic();
DROP FUNCTION IF EXISTS create_superadmin_user(text, text, text);

-- Step 3: Drop existing triggers
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
DROP TRIGGER IF EXISTS update_profiles_updated_at ON profiles;
DROP FUNCTION IF EXISTS handle_new_user();
DROP FUNCTION IF EXISTS create_supplier_for_supplier_profile();
